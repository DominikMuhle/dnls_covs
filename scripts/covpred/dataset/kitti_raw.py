...
def load_sequence(
    sequence_dir: Path, image_folder: str, matches_dir: Path, poses_file: str
) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
    K_matrix = load_camera_calibration_kitti_raw(sequence_dir.parent)
    poses = load_poses(matches_dir, poses_file)
    indices = image_indices(sequence_dir, image_folder)
    return poses, K_matrix, indices


class KITTIRawPreCompDataset(PreCompMatchesDataset):
    def __init__(
        self,
    ...
    def load_sequence(self, sequence_dir: Path) -> Tuple[torch.Tensor, torch.Tensor, List[int]]:
        ...
        return load_sequence(
            sequence_dir,
            self.dataset_config.directories.images,
            Path(self.dataset_config.directories.matches).joinpath(sequence),
            self.dataset_config.directories.poses_filename,
        )


class KITTIRawLiveDataset(LiveMatchesDataset):
